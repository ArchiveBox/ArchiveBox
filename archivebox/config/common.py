...
def base_url_from_legacy_server_config(raw_config: Mapping[str, object]) -> str:
    ...
    if listen_host and listen_host.lower() not in {"archivebox.localhost:8000", "archivebox.localhost:5797"}:
    ...
    nondefault_csrf_origins = [
        origin
        for origin in csrf_origins
        if origin not in {"http://admin.archivebox.localhost:8000", "http://admin.archivebox.localhost:5797"}
    ]
