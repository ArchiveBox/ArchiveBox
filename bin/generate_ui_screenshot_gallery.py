...
def build_gallery(manifest_path: Path, html_path: Path) -> None:
    ...
    for capture in grouped_captures:
        ...
        for profile, (width, height) in CAPTURE_PROFILES.items():
            ...
            screenshot_path = html_path.parent / filename
    ...
    if not allow_partial:
        ...
        for screenshot_path in html_path.parent.glob("*.png"):
...
def main() -> None:
    ...
    if len(sys.argv) == 4 and sys.argv[1] == "build":
        build_gallery(Path(sys.argv[2]), Path(sys.argv[3]))
        return
    raise SystemExit(
        "usage: generate_ui_screenshot_gallery.py append MANIFEST SCREENSHOT | "
        "validate SCREENSHOT_JSON EXPECTED_PATH | build MANIFEST HTML",
    )
